...
this year. I think we can't assume AVX.
...
`base`, `index` and `scale` all come from the `SIB` byte; if it is absent then
`rm` is used instead of `base` and zero is used instead of `index`.
...
The `ModR/M` byte has three fields:
 ...
 or `R12`):
   - 00 - I recommend we only use this in combination with a `SIB` byte, or for
...
ROM     0xC4FF40    rm      Set RIP to register (i.e. indirect jump)
RO      0xE840      imm32   Push RIP then JMP.  (i.e. direct call) (imm32)
ROM     0xC2FF40    rm      Push RIP then JMP (i.e. indirect call)
RO      0xC340              Pop RIP.
````
...
there are two main 64-bit calling conventions in use. I'm not sure how much to trust Wikipedia.
...
"Pop" means load the destination value from SP then add 8 to SP. If popping SP, it is written after it is incremented.
